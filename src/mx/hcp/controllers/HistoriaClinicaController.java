...
public class HistoriaClinicaController {
	...
	public ModelAndView create(@ModelAttribute("paciente") PacienteSession paciente, @ModelAttribute("usuario") UsuarioSession usuario) {
		...
		try {
			...
			try {
				aux = pacienteRepository.findOne(paciente.getId()).getHistoriacli().trim();
				aux = aux.isEmpty() ? null : aux;
	...
	@RequestMapping(value = "legacy/{idPaciente}", method = RequestMethod.GET)
	public ModelAndView legacy(@PathVariable("idPaciente") long idPaciente) {
		ModelAndView view = new ModelAndView("historia_clinica/legacy");
		view.addObject("historiaClinica", pacienteRepository.findOne(idPaciente).getHistoriacli().trim());
		return view;
	}
}
